...
bool GuiSpinner(Rectangle bounds, const char *text, int *value, int minValue, int maxValue, bool editMode)
{
    ...
    if (GuiButton(rightButtonBound, ">"))
        tempValue++;
#else
    if (GuiButton(leftButtonBound, GuiIconText(RAYGUI_ICON_ARROW_LEFT_FILL, NULL)))
        tempValue--;
    if (GuiButton(rightButtonBound, GuiIconText(RAYGUI_ICON_ARROW_RIGHT_FILL, NULL)))
        tempValue++;
#endif
...
static int GetTextWidth(const char *text)
{
    ...
    if ((text != NULL) && (text[0] != '\0'))
    {
        ...
        if (textIconOffset > 0)
            size.x += (RAYGUI_ICON_SIZE - 4); // RAYGUI_ICON_TEXT_PADDING
    }
...
#define RAYGUI_ICON_TEXT_PADDING 4
    ...
    if ((text != NULL) && (text[0] != '\0'))
    {
        ...
        if (iconId >= 0)
        {
            ...
            if ((text != NULL) && (text[0] != '\0'))
                textSize.x += RAYGUI_ICON_TEXT_PADDING;
        }
